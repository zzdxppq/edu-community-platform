...
:root {
  /* ===== 主色 Primary (蓝色系) ===== */
  --primary-900: #1a3d6e;      /* 深蓝 - 强调/hover状态 */
  --primary-800: #2C5AA0;      /* 主蓝 - 导航栏/标题/按钮 */
  --primary-700: #3568b2;      /* 中蓝 - 链接悬停 */
  --primary-600: #4a7cc4;      /* 浅蓝 - 选中状态 */

  /* ===== 强调色 Accent ===== */
  --accent-700: #3558a8;       /* 深强调 - 重要操作 */
  --accent-600: #426CC6;       /* 主强调 - 可点击元素/高亮 */
  --accent-500: #5580d4;       /* 中强调 - 悬停状态 */
  --accent-400: #7a9fe0;       /* 浅强调 - 辅助信息 */
...
| 场景 | 颜色变量 | 色值 | 用途说明 |
|------|----------|------|----------|
| **导航栏背景** | `--primary-800` | #2C5AA0 | 顶部固定导航 |
| **导航栏文字** | `--neutral-50` | #FFFFFF | 白色文字 |
| **侧边栏标题背景** | `--primary-800` | #2C5AA0 | 分类栏目标题 |
| **主标题** | `--neutral-900` | #1A1A1A | 页面主标题 |
| **正文文字** | `--neutral-800` | #333333 | 段落文本 |
| **链接文字** | `--accent-600` | #426CC6 | 可点击链接 |
| **链接悬停** | `--primary-700` | #3568B2 | hover状态 |
| **页面背景** | `--neutral-200` | #F2F2F2 | 整体页面背景 |
| **内容区背景** | `--neutral-50` | #FFFFFF | 白色内容卡片 |
| **分隔线** | `--neutral-400` | #CCCCCC | 虚线分隔符 |
| **日期/标签背景** | `--primary-800` | #2C5AA0 | 时间标签 |
| **按钮主色** | `--primary-800` | #2C5AA0 | 主要操作按钮 |
| **按钮强调** | `--accent-600` | #426CC6 | 强调操作按钮 |

### 2.3 配色对比示例

```
┌─────────────────────────────────────────────────────────────┐
│  [导航栏] 背景: #2C5AA0  文字: #FFFFFF                        │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  ┌──────────────┐  ┌────────────────────────────────────┐  │
│  │ [侧边分类导航]│  │          [内容区域]                 │  │
...
│  [LOGO] 河南城乡学校共同体发展平台                                │
│                                                                 │
│  首页  项目介绍  政策文件  示范校共体  新闻资讯  资源共享  [🔍] [登录]│
└─────────────────────────────────────────────────────────────────┘
```
...
```css
.header {
  background: var(--primary-800);    /* #2C5AA0 */
  height: 80px;
  position: fixed;
  top: 0;
  width: 100%;
...
.nav-link:hover,
.nav-link.active {
  background: var(--primary-900);    /* #1a3d6e */
}
```
...
┌────────────────────┐
│ ┌────────────────┐ │
│ │   栏目标题     │ │  ← 背景: #2C5AA0, 文字: #fff
│ └────────────────┘ │
│                    │
│  • 子栏目一        │  ← 文字: #333, 左侧边框: #426CC6
│  ─────────────────│  ← 虚线分隔: #ccc
│  • 子栏目二        │
│  ─────────────────│
│  • 子栏目三        │
...
.sidebar-item.active {
  ...
  border-left: 3px solid var(--accent-600);  /* #426CC6 */
...
/* 强调按钮 */
.btn-accent {
  background: var(--accent-600);     /* #426CC6 */
  color: var(--neutral-50);
  border: none;
}

.btn-accent:hover {
  background: var(--accent-700);     /* #3558a8 */
}
...
│  超出部分省略...                    │
│                                     │
│  2025-03-15  [查看详情 →]           │  ← 日期: #999, 链接: #1d4e92
└─────────────────────────────────────┘
```
...
.form-input:focus {
  ...
  box-shadow: 0 0 0 3px rgba(44, 90, 160, 0.15);  /* #2C5AA0 with opacity */
}
...
:root {
  /* 后台主色 - 与前台统一 */
  --admin-primary: #2C5AA0;    /* 主蓝色 */
  --admin-accent: #426CC6;     /* 强调色 */
  --admin-sidebar-bg: #1a3d6e; /* 深蓝侧边栏 */
